# -*- coding: utf-8 -*-
"""
Load Parquet files from a user-provided GCS path and write into an Iceberg table (event_summary).
Usage:
  pyspark <spark-options> -- /path/to/script.py \
    --path gs://bucket/prefix/2025/09/17/09/*/*.parquet \
    --table dpi_catalog.default.eco_event_summary_pt1m_dist
"""
...
def create_spark_session(app_name: str = "ParquetToIceberg-EVENT") -> SparkSession:
...
def parse_args():
    p = argparse.ArgumentParser(description="Load Parquet into an Iceberg table (event_summary).")
    p.add_argument("--path", type=str, required=True)
    p.add_argument("--table", type=str, default="dpi_catalog.default.eco_event_summary_pt1m_dist")
    return p.parse_args()
...
def main():
    ...
    columns_to_write = [
        "timestampMs","eventName","eventCategory","customerId","clientId","sessionId","inSession","userSessionId",
        ...
        "timezoneOffsetMins","connType","host","path","referrer","title","referrerHost","impactedFlow",
        "flowTagIntGroup1","flowTagFloatGroup1","watermarkMs","partitionId","retentionDate",
        "tagGroup1","tagGroup2","tagGroup3","tagGroup4","tagGroup5","tagGroup6","tagGroup7","tagGroup8","tagGroup9",
        "tagGroup10","tagGroup11","tagGroup12","tagGroup13","tagGroup14","tagGroup15",
